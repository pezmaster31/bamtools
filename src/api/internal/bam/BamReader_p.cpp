...
{
    if (a.RefID < 0 || a.Position < 0 || a.SupportData.NumCigarOperations == 0) {
        return false;
    }
    ...
    unsigned cigar1 =
        (unsigned)p[0] | (unsigned)p[1] << 8 | (unsigned)p[2] << 16 | (unsigned)p[3] << 24;
    if ((cigar1 & 0xf) != 4 || cigar1 >> 4 != a.SupportData.QuerySequenceLength) {
        return false;
    }
...
bool BamReaderPrivate::LoadNextAlignment(BamAlignment& alignment)
{
    ...
    if (m_stream.Read(allCharData.Buffer, dataLength) == dataLength) {
        ...
        for (unsigned int i = 0; i < alignment.SupportData.NumCigarOperations;
            ...
            if (m_isBigEndian) {
                BamTools::SwapEndian_32(cigarData);
            }
...
bool BamReaderPrivate::LoadReferenceData()
{
    ...
    if (m_isBigEndian) {
        BamTools::SwapEndian_32(numberRefSeqs);
    }
    ...
    for (unsigned int i = 0; i != numberRefSeqs; ++i) {
        ...
        if (m_isBigEndian) {
            BamTools::SwapEndian_32(refNameLength);
        }
        ...
        if (m_isBigEndian) {
            BamTools::SwapEndian_32(refLength);
        }
