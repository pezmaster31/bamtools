// ***************************************************************************
// PBgzfStream_p.cpp (c) 2011 Derek Barnett
...
#include "api/internal/io/pbgzf/PBgzfStream_p.h"
...
// ---------------------------
// PBgzfStream implementation
// ---------------------------

// constructor
PBgzfStream::PBgzfStream(void)
     /*
  : m_blockLength(0)
  , m_blockOffset(0)
  , m_blockAddress(0)
...
bool PBgzfStream::CheckBlockHeader(char* header) {
...
size_t PBgzfStream::DeflateBlock(int32_t blockLength) {
    // Do nothing
    return 0;
    /*
    ...
    while ( true ) {
        ...
        int status = deflateInit2(&zs,
                                  ...
                                  Z_DEFAULT_STRATEGY);
        if ( status != Z_OK )
            throw BamException("PgzfStream::DeflateBlock", "zlib deflateInit2 failed");
        ...
        if ( status != Z_STREAM_END ) {
            ...
            if ( status == Z_OK ) {
                inputLength -= 1024;
                if ( inputLength < 0 )
                    throw BamException("PgzfStream::DeflateBlock", "input reduction failed");
                continue;
            }

            throw BamException("PgzfStream::DeflateBlock", "zlib deflate failed");
        }

        // finalize the compression routine
        status = deflateEnd(&zs);
        if ( status != Z_OK )
            throw BamException("PgzfStream::DeflateBlock", "zlib deflateEnd failed");
        ...
        if ( compressedLength > Constants::BGZF_MAX_BLOCK_SIZE )
            throw BamException("PgzfStream::DeflateBlock", "deflate overflow");
    ...
    if ( remaining > 0 ) {
        if ( remaining > inputLength )
            throw BamException("PgzfStream::DeflateBlock", "after deflate, remainder too large");
...
void PBgzfStream::FlushBlock(void) {
    // Do nothing
    /*
    BT_ASSERT_X( m_device, "PgzfStream::FlushBlock() - attempting to flush to null device" );
    ...
    while ( m_blockOffset > 0 ) {
        ...
        if ( numBytesWritten < 0 ) {
            ...
            throw BamException("PgzfStream::FlushBlock", message);
        }
        ...
        if ( numBytesWritten != static_cast<int64_t>(blockLength) ) {
            ...
            throw BamException("PgzfStream::FlushBlock", s.str());
        }
...
size_t PBgzfStream::InflateBlock(const size_t& blockLength) {
    // Do nothing
    return 0;
    /*
    ...
    if ( status != Z_OK )
        throw BamException("PgzfStream::InflateBlock", "zlib inflateInit failed");

    // decompress
    status = inflate(&zs, Z_FINISH);
    if ( status != Z_STREAM_END ) {
        inflateEnd(&zs);
        throw BamException("PgzfStream::InflateBlock", "zlib inflate failed");
    }
    ...
    if ( status != Z_OK ) {
        inflateEnd(&zs);
        throw BamException("PgzfStream::InflateBlock", "zlib inflateEnd failed");
    }
...
bool PBgzfStream::IsOpen(void) const {
    /*
    if ( m_device == 0 )
        return false;
...
void PBgzfStream::Open(const string& filename, const IBamIODevice::OpenMode mode) {
    char mode_cstr[16] = "\0";
    ...
    // close current device if necessary
    Close();
    BT_ASSERT_X( (m_device == 0), "PgzfStream::Open() - unable to properly close previous IO device" );

    // retrieve new IO device depending on filename
    m_device = BamDeviceFactory::CreateDevice(filename);
    BT_ASSERT_X( m_device, "PgzfStream::Open() - unable to create IO device from filename" );
    ...
    if ( !m_device->Open(mode) ) {
        ...
        throw BamException("PgzfStream::Open", message);
    }
    */
}

// reads BGZF data into a byte buffer
size_t PBgzfStream::Read(char* data, const size_t dataLength) {
    ...
    BT_ASSERT_X( m_device, "PgzfStream::Read() - trying to read from null device");
...
// reads a BGZF block
void PBgzfStream::ReadBlock(void) {
    // Do nothing
    /*
    BT_ASSERT_X( m_device, "PgzfStream::ReadBlock() - trying to read from null IO device");
    ...
    if ( numBytesRead < 0 ) {
        const string message = string("device error: ") + m_device->GetErrorString();
        throw BamException("PgzfStream::ReadBlock", message);
    }
    ...
    if ( numBytesRead != static_cast<int8_t>(Constants::BGZF_BLOCK_HEADER_LENGTH) )
        throw BamException("PgzfStream::ReadBlock", "invalid block header size");

    // validate block header contents
    if ( !PBgzfStream::CheckBlockHeader(header) )
        throw BamException("PgzfStream::ReadBlock", "invalid block header contents");
    ...
    if ( numBytesRead != static_cast<int64_t>(remaining) )
        throw BamException("PgzfStream::ReadBlock", "could not read data from block");
...
void PBgzfStream::Seek(const int64_t& position) {

    // TODO: asserts
    pbgzf_seek(m_pbgzf, position, SEEK_SET);

    /*
    BT_ASSERT_X( m_device, "PgzfStream::Seek() - trying to seek on null IO device");
    ...
    if ( m_device->IsRandomAccess() && m_device->Seek(blockAddress) ) {
        ...
        throw BamException("PgzfStream::Seek", s.str());
    }
    */
}

void PBgzfStream::SetWriteCompressed(bool ok) {
    m_isWriteCompressed = ok;
}

// get file position in BGZF file
int64_t PBgzfStream::Tell(void) const {
    if ( NULL == m_pbgzf )
        return 0;
...
size_t PBgzfStream::Write(const char* data, const size_t dataLength) {
    
    // TODO: asserts
    return (size_t)pbgzf_write(m_pbgzf, data, dataLength);

    /*
    BT_ASSERT_X( m_device, "PgzfStream::Write() - trying to write to null IO device");
    BT_ASSERT_X( (m_device->Mode() == IBamIODevice::WriteOnly),
                 "PgzfStream::Write() - trying to write to non-writable IO device");
